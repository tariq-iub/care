                                                    ...
                                                    @else
                                                        ...
                                                        @if($option->answer_type == 'number' || $option->answer_type == 'text')
                                                            <div class="form-group d-flex mb-3">
                                                                ...
                                                                @if ($option->input_count == null || $option->input_count == 0)
                                                                    <input type="number" class="form-control me-2" id="input{{ $option->id }}" name="input{{ $option->id }}" value="{{ array_key_exists($option->id, $selected_option_id) ? $selected_option_id[$option->id] : '' }}" required>
                                                                @else
                                                                    @for ($i = 0; $i < $option->input_count; $i++)
                                                                        <input type="number" class="form-control me-2" id="input{{ $option->id }}index{{$i}}" name="input{{ $option->id }}index{{$i}}" value="{{ array_key_exists(str($option->id).str($i), $selected_option_id) ? $selected_option_id[str($option->id).str($i)] : '' }}" required>
                                                                    @endfor
                                                                @endif
                                                            </div>
                                                        @elseif($option->answer_type == 'checkbox')
                                                            <div class="form-check">
                                                                ...
                                                                <input class="form-check-input" id="checkbox{{ $option->id }}" type="{{$option->answer_type}}" name="checkbox{{ $option->id }}" value="1" {{ array_key_exists($option->id, $selected_option_id) && $selected_option_id[$option->id] == 1 ? 'checked' : '' }}>
                                                        ...
                                                        @else
                                                            @if ($option->radio_group)
                                                                ...
                                                                <div class="form-check ms-10">
                                                                    <input class="form-check-input" id="flexRadioDefault{{ $group }}{{ $question_id }}{{ $option->id }}" type="{{$option->answer_type}}" name="flexRadioDefault{{ $group }}{{ $option->radio_group }}{{ $question_id }}" value="{{ $option->id }}" {{ array_key_exists($option->id, $selected_option_id) ? 'checked' : '' }}>
                                                                    <label class="form-check-label fs-8" for="flexRadioDefault{{ $group }}{{ $question_id }}{{ $option->id }}">{{ $option->body }}</label>
                                                                </div>
                                                            @else
                                                                <div class="form-check">
                                                                    <input class="form-check-input" id="flexRadioDefault{{ $group }}{{ $question_id }}" type="{{$option->answer_type}}" name="flexRadioDefault{{ $group }}{{ $question_id }}" value="{{ $option->id }}" {{ array_key_exists($option->id, $selected_option_id) ? 'checked' : '' }}>
