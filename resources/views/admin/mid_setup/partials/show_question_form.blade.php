                                        ...
                                        @foreach($question_answers as $qa)
                                            @foreach($answers as $option)
                                                @if($qa->mid_answer_id == $option->id)
                                                    <input type="hidden" class="form-control" id="question{{$question_id}}" name="question{{ $question_id }}" value="{{$question_id}}" readonly>
                                                    ...
                                                    @elseif($option->answer_type == 'checkbox')
                                                        <div class="form-check">
                                                            <input class="form-check-input" id="checkbox{{ $option->id }}" type="{{$option->answer_type}}" name="checkbox{{ $option->id }}" value="1" {{ array_key_exists($option->id, $selected_option_id) && $selected_option_id[$option->id] == 1 ? 'checked' : '' }} onclick="return false;">
                                                            <label class="form-check-label fs-8" for="checkbox{{ $option->id }}">{{ $option->body }}</label>
                                                        </div>
                                                    @else
                                                        <div class="form-check">
                                                            <input class="form-check-input" id="flexRadioDefault{{ $option->id }}" type="{{$option->answer_type}}" name="flexRadioDefault{{ $question_id }}" value="{{ $option->id }}" {{ array_key_exists($option->id, $selected_option_id) ? 'checked' : '' }} onclick="return false;">
